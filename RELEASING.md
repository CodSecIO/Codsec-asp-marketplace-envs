# Releasing the ASP Terraform Marketplace product (internal runbook)

> Maintainer documentation - kept OUTSIDE `terraform-marketplace/` on purpose:
> `terraform-marketplace/README.md` is a **required file in the module ZIP**,
> ships to customers, and feeds the `cft` metadata title and description.
> Keep internal content out of it. All commands below run from
> `terraform-marketplace/`.

The product: customers deploy ASP through Infrastructure Manager. It is a
**separate product** from the classic deployer in `marketplace/` (one listing
cannot offer both deployment methods); both share the chart at
...
Redis). By default it is fully self-contained: it **creates** a dedicated
VPC (with GKE secondary ranges and Private Services Access) and a GKE
cluster. Existing infrastructure is opt-in: `create_cluster=false` +
`cluster_name`, `create_network=false` + network inputs (existing network
must already have PSA). `modules/` holds vendored copies of our production
modules from `CodSecIO/terraform-modules` (private repo, so the code ships
in the package) - every adjustment is commented in place.
